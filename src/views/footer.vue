<template>
    <footer>
        <div class="footer-logos">
            <div class="layout-icon" >
           <div class="icon1 grid--1"><img src="/src/views/image/image 25.png" alt="1"><img src="/src/views/image/image 32.png" alt="6"></div>
           <div class="icon2 grid--1"><img src="/src/views/image/image 26.png" alt="2"><img src="/src/views/image/image 33.png" alt="7"></div>
           <div class="icon3 grid--1"><img src="/src/views/image/image 27.png" alt="3"><img src="/src/views/image/image 34.png" alt="8"></div>
           <div class="icon4 grid--1"><img src="/src/views/image/image 28.png" alt="4"><img src="/src/views/image/image 35.png" alt="9"></div>
           <div class="icon5 grid--1"><img src="/src/views/image/image 29.png" alt="5"><img src="/src/views/image/image 36.png" alt="10"></div>
          </div>
      

        <div class="footer-last grid-layout">
          
          <div class="last1">
              <img src="/src/views/image/image 26.png" alt="logo">
              <p>Address<br>
                  Store & Office<br>
                  Jl. Setrasari Kulon III, No. 10-12,<br> Sukarasa, Sukasari, Bandung, <br>Jawa Barat, Indonesia 40152</p><br>
                  <p>Office Hour<br>
                      Monday - Sunday<br>
                      10.00 - 18.00</p>
          </div>
          <div class="last2 ">
             <h2 style="color:#ffff;padding-bottom:30px;margin-top:37px;font-size:30px"><span style="border:1px solid #ffffff;border-top:none;border-right:none;border-left:none;">Get</span> in touch</h2>
              <div class="p2 ">
              <p>Phone<br>
                  Service<br> Center<br>
                  Customer<br>
                  Service</p><br>

                  <p style="line-height: 33px">022-20277564 <br>
                      0811-233-8899 <br>
                  <span >0811-235-9988</span> </p>
              </div>



                 <div class="footer-iconf">
              <img src="/src/views/image/Facebook (1).png" alt="face">
             <img src="/src/views/image/Instagram (1).png" alt="img">
           <img src="/src/views/image/Twitter.png" alt="tw">
           <img src="/src/views/image/Youtube.png" alt="yu">
               
                 </div>
                  
          </div>
          <div class="last3">
            <h2 style="color:#ffff;padding-bottom:30px;margin-top:37px;font-size:30px"><span style="border:1px solid #ffffff;border-top:none;border-right:none;border-left:none;">Useful</span>  Link</h2>
              <p>Warranty & Complaints<br>
                  Order & Shipping<br>
                  Tracking Order<br>
                  About Us<br>
                  Repair<br>
                  Terms<br>
                  FAQ<br>
                  Service</p>

          </div>
          <div class="last4">
            <h2 style="color:#ffff;padding-bottom:30px;margin-top:37px;font-size:30px"><span style="border:1px solid #ffffff;border-top:none;border-right:none;border-left:none;">Camp</span>aign  </h2>
              <p>Mengenal Arti Cukup<br>
                  Tell Your Difference<br>
                  Waykambas<br>
                  Rebrand<br>
                  Gallery <br>
                  Singo <br>
                  Rakai <br>
                  </p>
          </div>
    
        </div>
      </div>
    </footer>
</template>

<script>
export default {
    setup () {
        

        return {}
    }
}
</script>

<style  scoped>
.layout-icon{
    display: flex;
    justify-content: center;
    align-items: center;
    gap:150px;
}
.footer-last{
    display: flex;
    justify-content: center;
    gap: 100px;
}
</style>